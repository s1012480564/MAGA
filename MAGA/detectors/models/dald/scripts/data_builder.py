...
import os
import json
import detectors.models.dald.scripts.custom_datasets
from datasets import load_dataset
from detectors.models.dald.scripts.model import load_tokenizer, load_model
...
class DataBuilder:
    ...
    # get ai-generated text for each original text through api call
    def _openai_sample(self, prefix):
        ...
        #openai - others
        elif self.args.openai_model in ['gpt-3.5-turbo-0301', 'gpt-3.5-turbo', 'gpt-4', 'gpt-4-0613','gpt-3.5-turbo-0613','gpt-3.5-turbo-1106','gpt-3.5-turbo-0125','gpt-4-turbo-2024-04-09','gpt-4-0125-preview','gpt-4-1106-preview']:
            roles = {'xsum': 'You are a News writer.',
                     'writing': 'You are a Fiction writer.',
                     'pubmed': 'You are a Technical writer.',
                     'german':'You are a writer.',
                     'apps': 'You are a Python engineer.'}
            prompts = {'xsum': 'Please write an article with about 150 words starting exactly with:',
                       ...
                       'apps': 'Please create a Python function to solve the following question with the following inputs and outputs:'
                       }
            messages = [
        ...
        #Claude3
        elif self.args.openai_model in ['claude-3-opus-20240229']:
            import anthropic
            ...
            #     {'role': 'user', 'content': f'{prompts[self.args.dataset]} {prefix}'},
            # ]
            # For close source training data generation
            messages = [
        ...
        #gemini
        elif self.args.openai_model in ["gemini-1.0-pro"]:
            import google.generativeai as genai
            from google.generativeai.types import HarmCategory, HarmBlockThreshold
            genai.configure(api_key=self.args.openai_key)
            generation_config = {
            ...
            model = genai.GenerativeModel(
                model_name=self.args.openai_model,
            ...
            prompts = {
                'xsum':'You are a News writer. Please write an article with about 150 words starting exactly with:',
            ...
            message = f'{prompts[self.args.dataset]} {prefix}'
            response = model.generate_content(message)
            
            print(response.prompt_feedback)
            text = response.text
            print("prefix is: ", prefix)
            if text.startswith(prefix[:20]):
                return text
            return prefix + ' ' + text
        else:
    ...
    def _sample_from_model(self, texts, min_words=55, prompt_tokens=30):
        ...
        if self.args.dataset in ['pubmed','apps']:
            ...
            for idx, prefix in enumerate(prefixes):
                while idx >= len(decoded):
                    try:
                        ...
                        print(ex)
                        print('Wait 5 minutes before retry ...')
                        time.sleep(300)
    ...
    def generate_training_samples(self):
        ...
        def filter_by_english(sample):
            return sample["language"] == 'English' and len(sample["conversation"][0]['content'].split()) < 300 and len(sample["conversation"][0]['content'].split()) > 50
        dataset = dataset.filter(filter_by_english)
        dataset = dataset.train_test_split(train_size=500)["train"]
        
        # self.base_model.eval()w
        ...
        def map_fn(data):
            ...
            try:
                outputs = self._openai_sample(texts)
            except Exception as ex:
                print(ex)
                print('Wait 2 minutes before retry ...')
                time.sleep(120)
                outputs = self._openai_sample(texts)
            ...
            #                                         eos_token_id=self.base_tokenizer.eos_token_id)
            # decoded = self.base_tokenizer.batch_decode(outputs, skip_special_tokens=True)
            # out["mistral-7b_output"] = decoded[0].replace(texts, "")
            out["claude3_output"] = outputs.replace(texts, "")
            out["claude3_prompt"] = texts
            return out
        dataset_mapped = dataset.map(map_fn)

        claude_dataset = load_dataset("Shengkun/Claude3-texts", split="train")

        dataset_concat = datasets.concatenate_datasets([claude_dataset, dataset_mapped])
        dataset_concat.push_to_hub("Shengkun/Claude3-texts")
