...
import argparse
import json
from detectors.models.dald.scripts.data_builder import load_data
from detectors.models.dald.scripts.model import load_tokenizer, load_model
from detectors.models.dald.scripts.metrics import get_roc_metrics, get_precision_recall_metrics
import pandas as pd
...
def experiment(args):
    ...
    for idx in tqdm.tqdm(range(n_samples), desc=f"Computing {name} criterion"):
        ...
        # original text
        tokenized = scoring_tokenizer(original_text, return_tensors="pt", padding=True, truncation=True, max_length=2048, return_token_type_ids=False).to(args.device)
        ...
        with torch.no_grad():
            ...
            if args.reference_model_name == args.scoring_model_name:
                ...
                tokenized = reference_tokenizer(original_text, return_tensors="pt", padding=True, truncation=True, max_length=2048, return_token_type_ids=False).to(args.device)
        ...
        with torch.no_grad():
            ...
            if args.reference_model_name == args.scoring_model_name:
                ...
                tokenized = reference_tokenizer(sampled_text, return_tensors="pt", padding=True, truncation=True, max_length=2048, return_token_type_ids=False).to(args.device)
                assert torch.all(tokenized.input_ids[:, 1:] == labels), "Tokenizer is mismatch."
                logits_ref = reference_model(**tokenized).logits[:, :-1]
...
def evaluation_sft_model(model, tokenizer, epoch, dataset, dataset_file, run_name=""):
    ...
    output_file = f"./exp_rebuttal/ours/{dataset_file}_llama2-7b_{run_name}.(5000_sft)"
