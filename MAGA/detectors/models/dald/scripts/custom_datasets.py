...
DATASETS = ['writing', 'english', 'german', 'pubmed', 'apps']
...
def load_apps(cache_dir):
    data = load_dataset("codeparrot/apps", "competition", split='train', cache_dir=cache_dir)
    # combine question input_output and solution
    data = [f'Question: {q} Input and Output: {i} Solution:{SEPARATOR}{s}' for q, i, s in zip(data['question'], data['input_output'], data['solutions'])]
    return data
